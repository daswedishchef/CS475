	...
	float * rs = new float [NUMTRIALS];
        ...
        for( int t = 0; t < NUMTRIES; t++ )
        {
                ...
                for( int n = 0; n < NUMTRIALS; n++ )
                {
                        ...
                        //If tt is less than 0., then the reflected beam went up instead of down. (Case C)
        ...
        avgProb = avgProb/NUMTRIES;
        printf("Threads: %d\nTrials: %d\nTries: %d\nMax Performance: %lf mTrials/s\navgProb: %lf\n",NUMT,NUMTRIALS,NUMTRIES,maxPerformance,avgProb);
        return 0;
}